...
<table class="table" cellspacing="1" cellpadding="2" width="80%" align="center" border="0">
  <tbody>
  <tr>
      <th class="bg_tr" align="center" colspan="7" height="25">教师管理</th>
      
    </tr>
  <tr>
   <td class="td_bg" align="left"  colspan="7" height="23"><a href="teacher_add.jsp">&nbsp;&nbsp;&nbsp; 新增教师   </a>
  	</td>
  </tr>
    <tr>
      ...
      <th class="bg_tr" align="center" width="150" height="25">职称</th>
    ...
    <% 
    	...
    	List<User> list=User.findAll(User.class,"u_type=?",new Object[]{"teacher"},"id desc",limit,start*(size-1));
    	for(User c:list){
     %>
    <tr>
      ...
      <a href="teacher_add.jsp?actionOption=update&id=<%=c.getId() %>" >修改</a></td>
      <td class="td_bg" align="center" height="23">
      <a href="teacher_temp.jsp?actionOption=delete&id=<%=c.getId() %>" >删除</a></td>
    </tr>
    <%}%>
    <tr>
    <% long total=User.count(User.class,"u_type=?",new Object[]{"teacher"}); %>
    <%   if(((total /limit)+1) == size){
    %>
    <td class="td_bg" align="right"  colspan="7" height="23">
    		<a href="teacher_list.jsp?start=0" >第一页</a>
    	
    		<a href="javascript:void(0)">下一页</a>
    		
    		<a href="teacher_list.jsp?start=<%=(size-1)*limit%>&size=<%=size-1%>">上一页</a>
    		
    		<a href="teacher_list.jsp?start=<%=total-(total %limit) %>&size=<%=(total /limit)+1 %>">尾页</a>
    		当前第:<%=size%>页
    		总计：<%=total %>条记录,
    		共<%=(total /limit)+1 %>页
    	</td>
    ...
    <td class="td_bg" align="right"  colspan="7" height="23">
    		...
    		<a href="teacher_list.jsp?start=<%=(size)*limit%>&size=<%=size+1%>">下一页</a>
